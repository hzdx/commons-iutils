...
public class EhCacheManger implements CacheManager {
	...
	public boolean add(String key, Object value, int timeToLive) {
		if ((key == null) || (value == null))
			return false;
		return this.ehcache
				.putIfAbsent(new Element(key, value, null, Integer.valueOf(0), Integer.valueOf(timeToLive))) == null;
	}
	...
	public long incr(String key, long delta, int timeToLive) {
		if ((key == null) || (delta == 0L))
			return -1L;
		if (this.ehcache != null) {
			Element element = this.ehcache.putIfAbsent(
					new Element(key, new Long(delta), null, Integer.valueOf(0), Integer.valueOf(timeToLive)));
			if (element == null) {
				return delta;
			}
